...
module.exports = async (client) => {
  try {
    ...
    for (const [guildId, applicationCommands] of Object.entries(
      applicationCommandsByGuild
    )) {
      ...
      for (const localCommand of localCommands) {
        ...
        if (maintenance === true) {
          logInfo(
            `Skipping command "${name}" for guild ${guildId} as it's on maintenance.`,
            "⏩"
          );
          continue;
        }
      ...
      for (const cmd of applicationCommands.cache.values()) {
        if (!localCommandNames.includes(cmd.name)) {
          ...
          logInfo(
            `Deleted command "${cmd.name}" from guild ${guildId} as it no longer exists locally.`,
            "🗑"
          );
        }
      }
